 ...
 */
export async function searchNearby(lng, lat, radius = 50000, types, page = 1, offset = 25, keywords) {
  if (!AMAP_KEY) return []
  ...
  try {
    ...
    if (data.status === '1' && data.pois) {
      ...
      // 四级处理：三级过滤 → 映射 → 去重
      const mapped = allPois
        .filter(p => !isReligiousOnly(p) && !isNonScenicJunk(p) && isScenicPOI(p))
        .map(poi => ({
          id: poi.id,
          name: poi.name,
          address: poi.address,
...
export const SCENIC_TYPES = '风景名胜|公园广场|纪念馆|国家级景点|省级景点|动物园|植物园|游乐园|水族馆'
...
// 190700=湿地公园 110101=公园
export const SCENIC_TYPE_CODES = '110000|110100|140100|140200|140300|140400|140500|140600|140700|080300|080400|141100|141200|141300|190700|110101'
...
function isNonScenicJunk(poi) {
  ...
  if (/酒店|宾馆|旅馆|民宿|饭店|餐厅|饭馆|酒家|食府|足浴|足道|洗浴|浴池|桑拿|KTV|网吧|棋牌/.test(name)) return true
