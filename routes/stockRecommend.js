...
router.get('/list', async (req, res) => {
  ...
  try {
    const { stocks, meta } = await buildScorePool(horizon, quickFilters, 300)
    ...
    list.sort((a, b) => {
      let va = a.basic[sortBy] ?? a[sortBy]
      let vb = b.basic[sortBy] ?? b[sortBy]
...
router.get('/detail/:code', async (req, res) => {
  ...
  try {
    const detail = await buildStockDetail(req.params.code, horizon)
    if (!detail) {
